...
app.get('/api/metrics/summary', authenticate, async (_req: Request, res: Response): Promise<void> => {
  try {
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`  POST   /api/metrics/api-tests  - Receive API test metrics`);
      console.log(`  GET    /api/metrics/api-tests  - Retrieve API test metrics`);
      console.log(`  POST   /api/metrics/e2e-tests  - Receive E2E test metrics`);
      console.log(`  GET    /api/metrics/e2e-tests  - Retrieve E2E test metrics`);
      console.log(`  GET    /api/metrics/summary    - Get metrics summary`);
      console.log('');
      console.log('Authentication: Bearer token required (set METRICS_API_KEY)');
